...
<div class="bg-white dark:bg-gray-700 rounded-xl shadow hover:shadow-lg transition cursor-pointer overflow-hidden" @click="$router.push(`/product/${product.id}`)">
    ...
    <div class="p-4">
      ...
      <h3 class="font-semibold text-gray-800 dark:text-white text-sm mb-2 line-clamp-2">{{ product.title }}</h3>
      ...
      <button
        ...
        class="mt-3 w-full bg-blue-600 text-white text-sm py-2 rounded-lg hover:bg-blue-700 transition">
        Add to Cart
      </button>
    </div>
  </div>
